...
if user_input:
    ...
    # Gemini sometimes returns content as a list of parts (e.g. when the
    # message is only a tool call with no text) instead of a plain string.
    ...
    if not final_answer:
        if getattr(final_message, "tool_calls", None):
            final_answer = (
                "⚠️ I hit the step limit before finishing (the model kept "
                "requesting tools without giving a final answer). Try "
                "rephrasing the question or breaking it into smaller parts."
            )
        else:
            final_answer = "⚠️ I couldn't produce an answer for that. Please try rephrasing."
